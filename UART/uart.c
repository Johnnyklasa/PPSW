...
// PINSEL
#define PIN00_SET_TX (1<<0)
#define PIN01_SET_RX (1<<2)
...
void UART_InitWithInt(unsigned int uiBaudRate){
   ...
   // UART0
   PINSEL0 = PINSEL0 | PIN01_SET_RX;                                  // ustawic pina na odbiornik uart0
   U0LCR  |= m8BIT_UART_WORD_LENGTH | mDIVISOR_LATCH_ACCES_BIT; // dlugosc slowa, DLAB = 1
   U0DLL   = (((15000000)/16)/uiBaudRate);                      // predkosc transmisji
   U0LCR  &= (~mDIVISOR_LATCH_ACCES_BIT);                       // DLAB = 0
   U0IER  |= mRX_DATA_AVALIABLE_INTERRUPT_ENABLE;               // uruchamia przerwanie informujoce ze ramka danych zostala zapisana i jest gotowa do odczytu
